...
```{ojs}
// Load the content index CSV
contentIndex = FileAttachment("data/contentIndex.csv").csv({ typed: true })

// Helper: get unique values for a column
unique = (arr, key) => Array.from(new Set(arr.map(d => d[key])));

// State: which tab is active? (default to 'MSP input data')
viewof activeTab = Inputs.radio([
  "MSP input data",
  "Knowledge base for decisions"
], { label: "Select data category:" })

// Filter contentIndex based on tab
sidebarItems = contentIndex.filter(d =>
  (activeTab === "MSP input data" && d.information_type === "input data") ||
  (activeTab === "Knowledge base for decisions" && d.information_type === "knowledge base")
)

// Render sidebar items
sidebarItems.map(d => `${d.nature} ([map](${d.mapLink})) ([doc](${d.documentLink}))`).join("\n\n")
```
