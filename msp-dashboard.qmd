...
```{ojs async}
// Load user_entry values from contentIndex.csv asynchronously
userEntries = (await FileAttachment("data/contentIndex.csv").csv({ typed: true })).map(row => row.user_entry)
policyMakerLabel = userEntries[1] // row 3, index 1
mspExpertLabel = userEntries[0]   // row 2, index 0
```
