...
cat("#", unlist(user_profile[4,"name"]), "\n\n")
...
below code should dynamically generate columns with content. does not work. The reason is that ojs code is executed after the document has been rendered, and not before, like is the case with R or Python. 
...
userProfile = FileAttachment("data/user_profile.csv").csv({ typed: true })
contentIndex = FileAttachment("data/contentIndex.csv").csv({ typed: true })

// needed: present links below with columne headers and two returns before each link presentation

// Extract values
contentIndex[0]?.mapLink ?? "";
contentIndex[0]?.documentLink ?? "";
